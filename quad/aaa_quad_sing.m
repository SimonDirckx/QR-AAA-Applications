...
alph = linspace(0,80,3000);
beta = [0,logspace(-14,-1,100)];

Fmat = [fs(xpts,alph),fl(xpts,beta)];
IF = [Is(alph),Il(beta)];
for j= 1:size(Fmat,2)
...
ii = find(real(pol_qr)>-1e-16 & real(pol_qr)<1 & abs(imag(pol_qr))<1e-15);
...
dlmwrite('quad_sing_poles.dat',[real(pol_qr),imag(pol_qr)]);
...
if use_exact_int
    ...
    for j=1:numel(w_qr)
        ...
        I_qr(j) = quadgk(r00,0,1,'RelTol',1e-13);
    end
    int_wghts_qr = I_qr;
    ii = find(abs(int_wghts_qr)<1e-15);
    z_qr(ii) = [];
...
err1(indtol) = test_accuracy('rootlog',z_qr,int_wghts_qr,false);
err2(indtol) = test_accuracy('besselj',z_qr,int_wghts_qr,false);
err3(indtol) = test_accuracy('bessely',z_qr,int_wghts_qr,false);
err4(indtol) = test_accuracy('log',z_qr,int_wghts_qr,false);
end
%%
figure(1);clf;
loglog(tolvec.',err1)
...
legend('rootlog','besselj','bessely','log')

sum_wghts = ones(1,size(W,1))*abs(W);
if use_exact_int
    dlmwrite('quad_sing_errors_use_exact_int.dat',[tolvec.',err1,err2,err3,err4]);
    dlmwrite('quad_sing_sum_wghts_use_exact_int.dat',[tolvec.',sum_wghts.']);
    dlmwrite('quad_sing_supp_exact_int.dat',z_qr);
else
    dlmwrite('quad_sing_errors.dat',[tolvec.',err1,err2,err3,err4]);
    dlmwrite('quad_sing_sum_wghts.dat',[tolvec.',sum_wghts.']);
